...
def main (args):
    ...
    for disco_lambda_reg_ in disco_lambda_regs:
        disco_lambda_str = meaningful_digits(disco_lambda_reg_).replace('.', 'p')
        disco_lambda_strs.append(disco_lambda_str)

        disco_var_ = "DisCo(#lambda={:s})".format(disco_lambda_str.replace('p', '.'))
        disco_vars.append(disco_var_)
        pass

    disco_var = disco_vars[disco_lambda_regs.index(disco_lambda_reg)]

    # Tagger feature collection
    tagger_features = ['Tau21','Tau21DDT', 'D2', kNN_var, 'D2', 'D2CSS', 'NN', ann_var, 'Adaboost', uboost_var, 'DisCo(#lambda=0)', disco_var]
    ...
    with Profile("Add variables"):
        ...
        with Profile("NN"):
            ...
            add_nn(data, classifier, 'DisCo(#lambda=0)')
            pass
        ...
        with Profile("DisCo"):
            for disco_var_, disco_lambda_str_ in zip(disco_vars, disco_lambda_strs):
                print "== Loading model for {}".format(disco_var_)
                #classifier = load_model('models/disco/classifier/full/classifier_lambda{}.h5'.format(disco_lambda_str_), custom_objects={'<lambda>':DisCo})
                classifier = load_model('models/disco/classifier/full/classifier_lambda{}.h5'.format(disco_lambda_str_), custom_objects={'<lambda>':binary_crossentropy})
                add_disco(data, classifier, disco_var_)
                pass
